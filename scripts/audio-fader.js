...
const MIN_CURVE_RESOLUTION = 64;
const MAX_CURVE_RESOLUTION = 256;
const SAMPLES_PER_SECOND = 64; // Target samples per second of fade duration

/**
 * Calculates the appropriate curve resolution based on fade duration.
 * Longer fades get more samples to avoid audible stepping artifacts.
 * @param {number} durationMs - Fade duration in milliseconds
 * @returns {number} Resolution (sample count) for the curve
 */
function calculateResolution(durationMs) {
  const durationSec = durationMs / 1000;
  const calculated = Math.round(durationSec * SAMPLES_PER_SECOND);
  return Math.min(MAX_CURVE_RESOLUTION, Math.max(MIN_CURVE_RESOLUTION, calculated));
}
 ...
 */
function generateExponentialCurve(startVol, targetVol, resolution) {
...
export function equalPowerCrossfade(soundOut, soundIn, duration) {
  ...
  // Calculate dynamic resolution based on crossfade duration
  const resolution = calculateResolution(duration);

  // Equal-power crossfade curves using trigonometric functions
  // These maintain constant perceived power: cos²(θ) + sin²(θ) = 1
  const curveOut = new Float32Array(resolution);
  const curveIn = new Float32Array(resolution);
...
export function scheduleEndOfTrackFade(ps) {
  ...
  debug(
    `[Fade] Scheduling end-of-track fade at ${fadeStartTime.toFixed(2)}s for "${ps.name
    }"`
  );
